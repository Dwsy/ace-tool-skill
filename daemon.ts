...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        :root {
            --bg-primary: #0a0a0a; --bg-secondary: #1a1a1a; --text-primary: #e5e5e5;
            --text-secondary: #666; --accent: #3b82f6; --border-color: #333;
        ...
        .results { background: var(--bg-secondary); padding: 1rem; border-radius: 6px; }
...
<body>
    <div class="container">
        ...
        <div class="section">
            <h2>Search</h2>
            <textarea id="searchQuery" rows="3" placeholder="Enter your search query..."></textarea>
        ...
        async function refreshStatus() {
            try {
                ...
                document.getElementById('status').textContent = 
                    data.status === 'running' ? '🟢 Online (Uptime: ' + data.uptime + 's)' : '🔴 Offline';
            } catch (e) {
        ...
        async function doSearch() {
            ...
            try {
                const res = await fetch('/call', {
                    ...
                    body: JSON.stringify({ method: 'tools/search', params: { query } })
                });
                const data = await res.json();
                if (data.result && data.result.results) {
                    results.innerHTML = data.result.results.map(r => 
                        '<div class="result-item"><strong>' + r.file + '</strong></div>'
                    ).join('');
                } else {
